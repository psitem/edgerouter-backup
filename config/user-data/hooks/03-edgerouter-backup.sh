...
sudo tar cf - -C / config | ssh -q -i $SSH_KEYFILE -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $SSH_USER@$SSH_HOST "gzip -cnq > $REPO_PATH/$FNAME_BACKUP" > /dev/null
